...
- `match_hotel()` is **Google-Places-driven**: it resolves the user's name via Places API v1 `:searchText` (`app/services/hotel/google_resolver.py`), then walks place_id exact-match → global rapidfuzz (WRatio cutoff 90) → illegal/unknown. The `lat`/`lng` parameters survive in the signature only for HTTP-contract back-compat (spec M0.4) and are ignored internally — Google's resolved coordinates win. **Out-of-Taipei resolutions return `unknown`, never `illegal`** (`legal_hotels` is Taipei-only; absence ≠ illegality outside the city). Backfill `legal_hotels.google_place_id` with `scripts/regeocode_hotels_google.py` after `ingest_hotels.py`; the column is owned by alembic `0004_legal_hotels_google_place_id`.
